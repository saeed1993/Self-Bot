...
local function action_by_reply(arg, data)
  ...
  if data.sender_user_id_ then
   ...
   if cmd == "unblock" or cmd == "رفع بلاک" then
...
if matches[1] == "clear cache" or matches[1] == "پاک کردن کش" and tonumber(msg.from.id) == our_id then
 ...
 if matches[1] == "block" and is_sudo(msg) or matches[1] == "بلاک" and is_sudo(msg) then
if not matches[2] and msg.reply_id then
 ...
 if matches[1] == "unblock" and is_sudo(msg) or matches[1] == "رفع بلاک" and is_sudo(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
1, nil, plug, '@s_kqz', dl_cb, nil)
    ...
    if matches[1]:lower() == 'save' and matches[2] and is_sudo(msg) or matches[1]:lower() == 'ذخیره' and matches[2] and is_sudo(msg) then
...
if matches[1] == 'delcontact' and is_sudo(msg) then
    ...
    if matches[1] == 'left' and is_sudo(msg) or matches[1] == 'لفت' and is_sudo(msg) then
    tdcli.sendMessage(msg.to.id, "", 0, "*Bye Bye :D*", 0, "md")
  tdcli.changeChatMemberStatus(chat, our_id, 'Left', dl_cb, nil)
   end

    if matches[1] == 'shahvar' or matches[1] == 'شاهوار' then
