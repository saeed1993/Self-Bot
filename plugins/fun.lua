do
--Begin Fun.lua By @s_kqz
--Special Thx To @s_kqz
...
if matches[1] == "helpfun" or matches[1] == "راهنمای سرگرمی" and is_sudo(msg) then
local text = [[
_Self Bot Fun Help Commands:_
_دستورات بخش سرگرمی سلف بات_

*!time*
*زمان*
_Get time in a sticker_
_دریافت زمان به صورت استیکر_
...
_ترجمه کردن انگیسی به فارسی یا بلعکس_
_Example:_
_مثال:_
*!tr fa hi*
*ترجمه fa hello*
...
tdcli.sendMessage(msg.from.id, 0, 1, text, 1, 'md')
	...
	if matches[1]:lower() == "calc" and is_sudo(msg) or matches[1]:lower() == "حساب" and matches[2] and is_sudo(msg) then 
...
--------------------------------
	if matches[1]:lower() == 'praytime' and is_sudo(msg) or matches[1]:lower() == 'اوقات شرعی' and is_sudo(msg) or matches[1] == 'azan' and is_sudo(msg) or matches[1] == 'اذان' and is_sudo(msg) then
		if matches[2] then
			city = matches[2]
...
--------------------------------
	if matches[1]:lower() == 'tophoto' and is_sudo(msg) or matches[1]:lower() == 'به عکس' and is_sudo(msg) and msg.reply_id then
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@s_kqz", dl_cb, nil)
					else
...
--------------------------------
	if matches[1]:lower() == 'tosticker' and is_sudo(msg) or matches[1]:lower() == 'به اسنیکر' and is_sudo(msg) and msg.reply_id then
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '@s_kqz', dl_cb, nil)
					else
...
--------------------------------
	if matches[1]:lower() == 'weather' and is_sudo(msg) or matches[1]:lower() == 'اب و هوا' and is_sudo(msg) then
		city = matches[2]
...
--------------------------------
	if matches[1]:lower() == 'time' and is_sudo(msg) or matches[1]:lower() == 'زمان' and is_sudo(msg) then
...
--------------------------------
if matches[1] == 'voice' and is_sudo(msg) or matches[1] == 'ویس' and is_sudo(msg) then
 local text = matches[2]
  ...
  local file = download_to_file(url,'Self-BotV2.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '@s_kqz', dl_cb, nil)
   end
end

 --------------------------------
	if matches[1] == "tr" and is_sudo(msg) or matches[1] == "ترجمه" and is_sudo(msg) then 
		url = https.request('https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20160119T111342Z.fd6bf13b3590838f.6ce9d8cca4672f0ed24f649c1b502789c9f4687a&format=plain&lang='..URL.escape(matches[2])..'&text='..URL.escape(matches[3]))
		data = json:decode(url)
		return 'زبان : '..data.lang..'\nترجمه : '..data.text[1]..'\n____________________\n @s_kqz :)'
	end
--------------------------------
	if matches[1]:lower() == 'short' and is_sudo(msg) or matches[1]:lower() == 'کوتاه' and is_sudo(msg) then
		...
		local opizo = http.request('http://api.gpmod.ir/shorten/?url='..URL.escape(shortlink)..'&username=hypeer.saeed@gmail.com')
...
--------------------------------
	if matches[1]:lower() == "sticker" and is_sudo(msg)  or matches[1]:lower() == "استیکر" and is_sudo(msg) then 
...
--------------------------------
	if matches[1]:lower() == "photo" and is_sudo(msg) or matches[1]:lower() == "عکس" and is_sudo(msg) then 
		local eq = URL.escape(matches[2])
		local w = "500"
		local h = "500"
		local txtsize = "100"
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@SHAHVARTEAM", dl_cb, nil)
	end
end
end
--------------------------------
