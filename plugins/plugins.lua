...
local function list_all_plugins(only_enabled)
  ...
  local text = text..'\n\n'..nsum..' *📂پلاگین های نصب شده*\n\n'..nact..' _➕پلاگین های فعال_\n\n'..nsum-nact..' _➖پلاگین های غیرفعال_'..tmp
  return text
end
...
local function run(msg, matches)
  ...
  if is_sudo(msg) then
  if matches[1]:lower() == '!plist' or matches[1]:lower() == '/plist' or matches[1]:lower() == '#plist'or matches[1]:lower() == 'لیست پلاگین' then --after changed to moderator mode, set only sudo
    return list_all_plugins()
  end
end
  -- Re-enable a plugin for this chat
   if matches[1] == 'pl' or matches[1] == 'پلاگین' then
  ...
  if matches[1]:lower() == 'reload'and is_sudo(msg) or matches[1]:lower() == 'بروز' and is_sudo(msg) then --after changed to moderator mode, set only sudo
    return reload_plugins(true)
  end
end
