...
local text = answer[math.random(#answer)]
			...
			if matches[1]:lower() == "addname" and is_sudo(msg) or matches[1]:lower() == "افزودن اسم" and is_sudo(msg) then
      local name = matches[2]
      return set_name(name)
      elseif matches[1]:lower() == "remname" and is_sudo(msg) or matches[1]:lower() == "حذف اسم" and is_sudo(msg) then
      local name = matches[2]
      return rem_name(name)
			elseif matches[1]:lower() == "setanswer" and is_sudo(msg) or matches[1]:lower() == "تنظیم پاسخ" and is_sudo(msg) then
      local answer = matches[2]
      return set_answer(answer)
      elseif matches[1]:lower() == "remanswer" and is_sudo(msg) or matches[1]:lower() == "حذف پاسخ" and is_sudo(msg) then
      local answer = matches[2]
      return rem_answer(answer)
         elseif matches[1]:lower() == 'namelist' and is_sudo(msg) or matches[1]:lower() == 'لیست اسم' and is_sudo(msg) then
  return tdcli.sendMessage(msg.to.id, msg.id, 0, namelist(msg), 0, "md")
         elseif matches[1]:lower() == 'answerlist' and is_sudo(msg) or matches[1]:lower() == 'لیست پاسخ' and is_sudo(msg) then
