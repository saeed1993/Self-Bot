local function show_bot_settings(msg)
   ...
   local hash = 'markread'
    if redis:get(hash) == "on" or redis:get(hash) == "روشن" then
        markread = '[روشن]'
    else
        markread = '[خاموش]'
    end
...
local hash = 'autoleave' 
	...
	if is_sudo(msg) then
	  if msg.content_.text_ == "/self on" or msg.content_.text_ == "/Self on" or msg.content_.text_ == "!self on" or msg.content_.text_ == "!Self on" or msg.content_.text_ == "سلف روشن" then
...
local receiver = msg.chat_id_
 ...
 if matches[1] == 'on' or matches[1] == 'روشن' then
  return enable_channel(receiver)
 end
 if matches[1] == 'off' or  matches[1] == 'خاموش' then
  return disable_channel(receiver)
 end
-----------------------
     if matches[1] == 'autoleave' and is_sudo(msg) or matches[1] == 'خروج خودکار' and is_sudo(msg) then
local hash = 'autoleave'
--Enable Auto Leave
     if matches[2] == 'on' or matches[2] == 'روشن' then
     if not redis:get(hash) then
   return 'خروج خودکار روشن شد'
      else
    redis:del(hash)
   return 'خروج خودکار روشن شد'
     end
--Disable Auto Leave
   ...
   return 'خروج خودکار خاموش شد'
     ...
     if matches[1] == 'antiflood' and is_sudo(msg) or matches[1] == 'ضداسپم' and is_sudo(msg) then
local hash = 'anti-flood'
--Enable Anti-flood
     if matches[2] == 'on' or matches[2] == 'روشن' then
  if not redis:get(hash) then
...
--Disable Anti-flood
  ...
  if redis:get(hash) then
   ...
   return '_Private_ *flood protection* _has been_ *disabled*'
                ...
                if matches[1] == 'pvfloodtime' and is_sudo(msg) or matches[1] == 'حساسیت اسپم' and is_sudo(msg) then
          ...
          elseif matches[1] == 'pvsetflood' and is_sudo(msg) or matches[1] == 'تنظیم حساسیت' and is_sudo(msg) then
                    if not matches[2] then
                    else
                        hash = 'flood_max'
                        redis:set(hash, matches[2])
       ...
       if matches[1] == 'settings' and is_sudo(msg) or matches[1] == 'تنظیمات' and is_sudo(msg) then
      return show_bot_settings(msg)
                 end

       if matches[1] == 'edit' and matches[2] and is_sudo(msg) or matches[1] == 'ویرایش' and matches[2] and is_sudo(msg) then
...
*دستورات:*
*تنظیمات*
_ارسال تنظیمات سلف بات_

*شناسه گروه*
_نمایش شناسه گروه_

*تبدیل به سوپرگروه*
_تبدیل کردن به سوپرگروه_
...
*!delmy*`[name | username]`
_پاک کردن اسم یا یوزرنیم_

*!markread* `[on | off]`
_فعال و غیرفعال کردن خواندن پیام ها_

*خروج خودکار* `[روشن | خاموش]`
_تنظیم خروج خودکار_
...
*!self* `[on | off]`
_فعال و غیرفعال کردن سلف_

*سنجاق* `(ریپلی)`
_سنجاق کردن پیام در گروه_

*حذف سنجاق*
_پاک کردن سنجاق گروه_

*شناسه* `[ریپلی | نام کاربری]`
_نمایش شناسه کاربر_

*حذف* (ریپلی)
_پاک کردن پیام_

*دعوت* `[شناسه | نام کاربری | ریپلی]`
_دعوت کاربر به گروه_

*اخراج* `[شناسه | نام کاربری | ریپلی]`
_اخراج کردن کاربر از گروه_

*حذف همه* `[شناسه | نام کاربری | ریپلی]`
_پاک کردن همه پیام های کاربر_
...
*تنظیم*`[اسم | توضیحات | لینک]`
_تنظیم کردن لینک/اسم/توضیحات گروه_

*!addplugin* _text_ `name.lua`
_ایجاد یک فایل پلاگین_

*!delplugin* `name`
_حذف کردن یک پلاگین_

*!setmy*`[name | username]` *(name|username)*
_تنظیم کردم اسم و نام کاربری_

*!addcontact* `[phone | firstname | lastname]`
_اضافه کردن یک مخاطب_

*!delcontact* `[phone]`
_حذف مخاطب_

*!addname* `[name]`
_اضافه کردن اسم به لیست_

*!remname* `[name]`
_حذف کردن اسم از لیست_

*!setanswer* `[answer]`
_اضافه کردن پاسخ به لیست_

*!remanswer* `[answer]`
_حذف کردن پاسخ از لیست_
...
*تنظیم حساسیت* `[عدد]`
_تنظیم حداکثر تعداد اسپم برثانیه_

*حساسبت اسپم* `[ثانیه]`
_تنظیم زمان چک اسپم_

*بلاک* `[ریپلی | شناسه | یوزرنیم_]`
_بلاک کردن کاربر_

*رفع بلاک* `[ریپلی | شناسه | یوزرنیم]`
_رفع بلاک کردن کاربر_

*!sendfile* `[folder] [file]`
_ارسال فایل از پوشه_

*!sendplug* `[plug]`
_ارسال پلاگین_

*!save* `[plugin name] [reply]`
_ذخیره کردن پلاگین با ریپلی_

*!savefile* `[adress/filename] [reply]`
_ذخیره کردن فایل با ریپلی در فولدر_

*ویرایش* `[متن] [ریپلی]`
_ویرایش کردن متن با ریپلی _

*پاک کردن کش*
_پاک کردن کش در مسیر .telegram-cli/data_
